...
<div class="container">
	...
	<h6 class="mb-5">Realice la asignación del paciente a
		continuación</h6>
	...
	<section class="">
		...
		<article class="">
			...
			<p class="mt-2">Datos del paciente a internar</p>

			<div class="">

				<div
					class="form-row d-flex justify-content-between flex-md-nowrap align-items-center pb-2 mb-4">
					...
					<div class="col">
						...
						<div class="">
							<label for="" class="h6 font-weight-bold">${paciente.getApellido()},
								${paciente.getNombre()}</label>
						</div>
						<div class="">
							<label for="" class="p ">${paciente.getTipoDocumento().name()}:
		...
		<article class="">
			...
			<p class="mt-2">Requisitos necesarios para realizar la
				internación</p>

			<div class="mt-2">
				<label for="" class="p">Tipo de cama:</label> <span class="p">${tipoCama.getDescripcion()}.</span>
			</div>
			<div class="">
				<label for="" class="p">Tipo de sala:</label> <span class="p">${tipoSala.getDescripcion()}.</span>
			</div>
			<div class="mb-4">
				<label for="" class="p">Urgencia:</label> <span class="p">${urgencia}.</span>
			</div>
		</article>
	...
	<p class="mt-2 mb-4">Datos de la instituciones para realizar la
		internación</p>
		...
		<table id="myTable"
			...
			<tr class="una bg-white">
				...
				<th style="vertical-align: middle; width: 16%"
					...
					onmouseout="this.style.backgroundColor='white';">NOMBRE <img
				...
				<th style="vertical-align: middle;" class="border border-secondary"
					onmouseover="this.style.backgroundColor='#dee2e6 ';"
					onmouseout="this.style.backgroundColor='white';">TIPO <img
					style="margin-top: 0.40rem;"
					class="bi bi-arrow-down-up float-right" alt="" src="img/sort.svg"
					width="10px" height="">
				...
				<th style="vertical-align: middle;" class="border border-secondary"
					onmouseover="this.style.backgroundColor='#dee2e6';"
					onmouseout="this.style.backgroundColor='white';">DISPONIBILIDAD
					<br>DE CAMAS <img style="margin-top: 0.40rem;"
					class="bi bi-arrow-down-up float-right" alt="" src="img/sort.svg"
					width="10px" height="">
			...
			<c:forEach items="${listaInstituciones}" var="institucion">
				...
				<tr
					...
					<form action="enviarAsignacion" method="GET">

						<input type="hidden" value="${paciente.getId()}" name="idPaciente">
						<input type="hidden"
							value="${institucion.getInstitucion().getId()}"
							name="idInstitucionAInternar"> <input type="hidden"
							value="${urgencia}" name="urgencia"> <input type="hidden"
							value="${Math.round(institucion.getDistancia())}"
							name="distancia">

						<td style="vertical-align: middle;"><c:out
								value="${institucion.getInstitucion().getNombre()}" /></td>
						<td style="vertical-align: middle;"><c:out
								value="${institucion.getInstitucion().getTipo().getDescripcion()}" /></td>
						<td style="vertical-align: middle;"><c:out
								value="${institucion.getInstitucion().getDomicilio().getLocalidad().getNombreLocalidad()}" /></td>
						<td style="vertical-align: middle;"><c:out
								value="${Math.round(institucion.getDistancia())} Km" /></td>
						...
						<td style="vertical-align: middle; text-align: left; width: 25%;"
							...
							<div class="d-flex justify-content-center">
								...
								<div>
									...
									<c:forEach items="${institucion.getListaSala()}" var="lista"
										...
										<c:if
											test="${institucion.getSalaYCamaRequerida()  == true
	               				&& lista.getSala().getTipoSala() == tipoSala}">
					               		...
					               		class="text-danger"
													...
													<c:out value="${lista.getCantidad()}" /> de <c:out
														value="${lista.getSala().getTipoSala().getDescripcion()}" />.

													<c:forEach items="${lista.getListaCama()}" var="listaCamas">
														<ul
															class="my-2 pl-3 d-flex justify-content-between align-items-center">
															...
															<span class="ml-5"> <input type="radio"
																name="idCama" value="${listaCamas.getCama().getId()}"
							...
							<p id="noTiene${institucion.getInstitucion().getId()}"
		               		...
		               		institucion.getSalaRequerida() != true }">
	               					...
	               					Esta institución <abbr class="font-weight-bold">NO
										TIENE</abbr> una sala de <abbr class="font-weight-bold">${tipoSala.getDescripcion()}</abbr> 
		               		...
		               		institucion.getSalaRequerida() != true }">
	               					...
	               					Esta institución <abbr class="font-weight-bold">NO
										TIENE</abbr> una sala de <abbr class="font-weight-bold">${tipoSala.getDescripcion()}</abbr>
	               					disponible con una cama <abbr
										class="font-weight-bold">${tipoCama.getDescripcion()}</abbr>.
               				...
               				institucion.getSalaRequerida() == true }">	
				               		...
				               		Esta institución <abbr class="font-weight-bold">NO
										TIENE</abbr> una cama <abbr class="font-weight-bold">${tipoCama.getDescripcion()}</abbr> 
				               		disponible en una sala de <abbr
										class="font-weight-bold">${tipoSala.getDescripcion()}</abbr>.
		               		...
		               		institucion.getSalaRequerida() != true }">
								<div class="d-flex justify-content-center">
									...
									<div id="mostrarInfo${institucion.getInstitucion().getId()}"
										hidden>
						               		...
						               		class="text-danger"
													...
													<c:forEach items="${lista.getListaCama()}" var="listaCamas">
														<ul
															...
															<span class="ml-5"> <input type="radio"
																name="idCama"
																id="activarBotones${institucion.getInstitucion().getId()}"
																value="${listaCamas.getCama().getId()}"
								...
								<div class="d-flex justify-content-center">
									<div class="custom-control custom-switch mt-2">
										<input type="checkbox" class="custom-control-input"
											id="activarInfo${institucion.getInstitucion().getId()}"
										...
										<label class="custom-control-label"
											for="activarInfo${institucion.getInstitucion().getId()}">Mostrar
											información</label>
									</div>
								</div>
								...
								<div class="modal-dialog modal-dialog-centered modal-lg"
									...
									<div class="modal-content  px-3">
										<div class="modal-header pb-2">
											<h5 class="modal-title" id="exampleModalLongTitleUno">Realizar asignación</h5>
										...
										<div class="modal-body text-left">
											...
											<p class="p mt-2 mb-4">
												Se realizará la asignación del paciente <span
											...
											<p class="mb-0 font-weight-lighter">Se notificará  a la institución de la reserva de cama correspondiente a la asignación.</p>
										</div>

										<div class="modal-footer">
											<button type="button" class="btn btn-outline-primary"
												data-dismiss="modal">Volver</button>
											<button type="submit" class="btn btn-outline-success">Realizar Asignación</button>
										</div>
									</div>
								</div>
